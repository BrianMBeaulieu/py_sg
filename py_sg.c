...
sg_read(PyObject *self, PyObject *args)
{
  ...
  if (PyObject_AsWriteBuffer(bufObj, (void*)&buf, &bufLen) < 0) {
    ...
    PyErr_Clear();
    //bufObj = PyStr_FromStringAndSize(NULL, bufLen); // new blank string
    bufObj = PyStr_FromStringAndSize(NULL, bufLen); // new blank string
    printf("%s",bufObj);
    if (!bufObj) return NULL;

    //buf = (unsigned char*)PyString_AS_STRING(bufObj);
    buf = (unsigned char*)PyStr_AsString(bufObj);
    newbuf = 1;
  }
  ...
  if (newbuf) {
    ...
    if (PyUnicode_Resize(&bufObj, len) < 0) return NULL;
    return bufObj;
  } else {
...
PyMODINIT_FUNC PyInit_py_sg(void)
{
